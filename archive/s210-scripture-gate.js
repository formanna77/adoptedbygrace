#!/usr/bin/env node
/* S210 lead gate (was S209) — verify EVERY quotation against scripture-niv.js ground truth.
...
const OUT=process.env.S210_OUT||path.join(__dirname,'s210-out');
...
for(const f of fs.readdirSync(OUT).filter(f=>f.endsWith('.txt'))){
 ...
 for(const b of txt.matchAll(/^=== PAGE: (.+?)\r?\n(?:(?!^=== PAGE:)[\s\S])*?^REPLACEMENT:\r?\n((?:(?!^=== PAGE:)[\s\S])*?)\r?\n^END_REPLACEMENT/gm)){
  ...
  // S210 — STRIP ATTRIBUTE VALUES BEFORE HUNTING FOR QUOTATIONS.
 // Every "needs-eyes" line in the first wave-2 run was an href: the matcher
 // looks for text between double quotes, and `href="/devotional-never-gives-up"`
 // is text between double quotes. Four of six lines in the report were that.
 // It matters because CHECK 17's own history is the argument: a Scripture gate
 // whose noise makes it unreadable "ran every session for months and never once
 // caused a correction." A gate nobody reads is a gate that does not exist.
 const stripped=repl.replace(/<blockquote>[\s\S]*?<\/blockquote>/g,'')
   .replace(/<[a-z][^>]*>/gi, m => m.replace(/="[^"]*"/g, '=""'));
