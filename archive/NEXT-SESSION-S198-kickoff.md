...
**Read `Session 198-PRE` first and read it carefully.** Six diagnostic passes ran between S197's close and your start. Three new checks, two rewritten build scripts, a new load-bearing ledger, two new standing tools, five new laws in CLAUDE.md, a corpus-wide heading fix, and a 192-item queue all landed in that window. Everything below assumes you know it.
...
8. **45 body-level heading skips, left for you on purpose.** The sixth pass fixed the footer cause (582 pages, `fix-heading-hierarchy.js`, guarded so the 29 footers with a real `<h3>` parent are untouched). What remains is 45 pages where the outline breaks *inside the argument* — `node fix-heading-hierarchy.js --report`. **Do not automate these.** Choosing between "this h4 should be an h3" and "this h3 should not exist" is a decision about what the section means, and a script that guesses is renumbering the structure of an argument. `404.html` and `all-content.html` (h1→h3) are the two utility cases and are trivial.
9. **One residual inconsistency from that fix, if you care to close it.** Converted footers render their column headings at 22.4px/600; the 29 guarded footers render theirs at 16.8px/700. Both are Playfair, both are correct-looking, neither overflows at 390 or 1196 — but they are not the same. `.footer-section h3` asks for 1.05rem and something later is winning at 1.4rem. Ten minutes with the browser open and the cascade in front of you; do not reason about it.
10. **Three small things the fifth pass found and left for you, deliberately.**
