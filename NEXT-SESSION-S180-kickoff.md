...
*S179 edited **zero pages** — all three cold-read agents died to API 529 / server errors. But it fixed two things that were silently costing every prior session: **(1) the site's INTERNAL STRATEGY DOCS were being served publicly** (`/CLAUDE.md` and `/VOICE.md` verified live over HTTP; all 148 root `.md` files reachable) — now blocked in `_redirects`; and **(2) the close detector's false-positive ROOT CAUSE was found and fixed.* **S178's "grid connector" diagnosis was wrong** — the real bug was container `<div>`s being scored as prose. **Queue 60 → 48, and NO PAGE NEEDS A MANUAL SKIP ANY MORE:** the-two-arms 16 → 1, objection-why-believe 14 → 0. The worst-first ordering is trustworthy for the first time. Profile before delegating anyway.*
