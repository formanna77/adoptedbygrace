...
## AUDIT 2 / Session 49 starts here

**🟢 PHASE F + PHASE G + LANDING-FORCE AUDIT CONTINUE IN PARALLEL. PHASE D EXPANSION REMAINS PAUSED.** Session 48 (2026-05-22) ran a pre-session deep-dive readiness audit (all GREEN — see the Session 48 Log entry) and then, on Aaron's "do it all" directive, retrofitted the "In Brief" 90-second-searcher on-ramp SITE-WIDE — 210 boxes (apologetics 4/36 → 36/36; site-wide class="tldr" 323 → 533; deep-article coverage 533/571 = 93%), substantially closing benchmarking Gap #1. Done via 5 parallel subagents under strict palette/discipline constraints; all 1,303 tldr hrefs verified resolving; lock count unchanged 298/289; all 8 validators GREEN; all 4 discipline axes still 0; audit-prose held at 169 clean. The only box-less article-body pages left are deliberate exclusions (utility, printables, 3 already-have-In-Brief, objection-builder, testimony-wall, verse-navigator, and 12 bespoke visual/interactive pages flagged for hand-tailoring). Session 47 (prior) closed at 8 article-equivalents (above floor 4, into ceiling-stretch): FIVE net-new Five-Point builds + THREE unmarked apex pages HAMMER-LOCKED. Net-new: `apologetic-sinful-from-birth.html` (TD #6 via Psalm 51:5 — the Hebrew *be‘awon cholalti*, sin pushed back to conception + the nursery mirror + Romans 5:12 / Augustine), `apologetic-save-his-people.html` (DA #6 via Matthew 1:21 — *sosei ton laon autou*, future-indicative + the name *Yeshua* as the argument), `apologetic-the-valley-of-dry-bones.html` (IG #6 via Ezekiel 37 — *ruach* ×10 + the two stages + preaching to corpses), `apologetic-the-deceitful-heart.html` (TD #7 via Jeremiah 17:9 — *aqob*/the Jacob root + the self-auditing trap + modern psychology), `apologetic-kept-by-the-power-of-god.html` (PoS #6 STRETCH via 1 Peter 1:3-5 — *teteremenen*/*phrouroumenous*, the double keeping). HAMMER-LOCKED this session: `systematic-reprobation`, `word-study-every-all`, `analogy-chess-grandmaster`. All eight pipeline validators GREEN; all four discipline axes clean site-wide (slogan-trap 0; clock-time 0; structural 0; "N in the morning" 0). **Lock count authoritative grep: 298 per-type (200 POLISH + 84 HAMMER + 14 CONSECRATED) / 289 distinct.** The 5 new apologetics are intentionally UNMARKED.

**Read these files FIRST, in order:**
1. **`MISSION-CONTROL.md`** Session 48 entry (above this block — the In Brief on-ramp retrofit + readiness audit) and the Session 47 entry below it. The Session 47 entry documents the five net-new builds, the all-PASS 6-page sample-read, the three apex HAMMER-LOCKs, and a lock-count note (S46 logged 182 POLISH but the live grep reads 200 — inherited drift to spot-check, NOT a Session-47 change). **Inheritance reminder: before any net-new build, Glob `apologetic-*.html` against the inventory below and verify any unexplained-recent files apex-grade before writing on the same topic (the Session-44 near-redo lesson).**
2. **`VOICE.md`** §XIII (Depravity Concrete-Unpacking Mandate) and §XIV (Five-Point Proliferation Mandate). Session 48 continues both. Five-Point coverage post-Session-47: **TD 7, UE 6, DA 6, IG 6, PoS 6 — 31 Five-Point apologetics on disk, every point at 6+.** The diamond is balanced; per the Landing-Force Standard, breadth may now begin pivoting from "more facets" toward "every prior facet polished to its sharpest form" — but fresh angles are still welcome where a point lacks them.
...
**Session 49 task — Phase F + Phase G + Landing-Force Audit, in parallel.** Floor: 4 article-equivalents (any axis counts). Stretch: 6-10. Ceiling-stretch: 12+. **Open by Glob-checking `apologetic-*.html` against the Session-47 inventory (31 Five-Point + 5 older = 36 total expected); verify any net-new files apex-grade before counting toward floor.**

**🆕 PRIORITY 0 — SAMPLE-READ AUDIT (mandatory, opens every session).** Draw a fresh random sample of 5–10 pages — half locked, half unmarked — and live-read each top-to-bottom against the landing-force test. Any page that fails goes onto the surgical-fix/rebuild queue; locks may be revoked. **ROTATE — already read and PASSED: S48 read systematic-hamartiology + pastoral-assurance (both PASS; confirmed the old 3:14-in-the-morning banned-move in pastoral-assurance is gone); S47 read systematic-regeneration, question-where-did-your-faith-come-from, devotional-in-christ, systematic-reprobation, word-study-every-all, analogy-chess-grandmaster; S46 read systematic-election, question-chosen, question-perseverance, systematic-tulip, analogy-adoption; S45 read devotional-never-gives-up, question-faithgift, question-depravity, analogy-doctor-corpse, psychology-system-justification, history-golden-thread.** Suggested fresh seed for S49: `question-irresistible`, `devotional-chosen-before-you-were-broken`, `two-arms` (HAMMER-LOCKED), `start-here-phase1`, one of the new In Brief apologetics read end-to-end (e.g. `apologetic-the-valley-of-dry-bones`), plus 2–3 random unmarked content pages. The live-read is the ceiling; the scanner is the floor.
...
**Priority 1 — Phase G Five-Point Proliferation continuation.** Post-Session-47 coverage:

| Point | Apologetics shipped | Status |
|---|---:|---|
| Total Depravity | 7 (Lazarus + cardiology + prayer-diagnostic + conscience + mind-cannot-submit + sinful-from-birth + deceitful-heart) | strongest |
| Unconditional Election | 6 (*eklogē* + Ephesians 1 eulogy + *tetagmenoi* + Deuteronomy 7 + those-you-gave-me + you-did-not-choose-me) | strong |
| Definite Atonement | 6 (priest-shoulders + Owen + mercy seat + *ephapax* + for-the-sheep + save-his-people) | strong |
| Irresistible Grace | 6 (Lydia + revival + cardiac transplant + *helkyō*/John 6 + Damascus + dry bones) | strong |
| Perseverance | 6 (*arrabōn* + unbroken chain + double-grip + nothing-can-separate + he-who-began + kept-by-the-power) | strong |

**Five-Point Proliferation is now balanced at SIX facets per point (TD at 7) — 31 Five-Point apologetics on disk.** Session 48's recommended Priority 1 candidates (each from an angle the site does not yet cover — **pre-Glob-verify each filename does not already exist before writing**):
- **Unconditional Election via Romans 11:5-6** — "a remnant chosen by grace... and if by grace, then it cannot be based on works; if it were, grace would no longer be grace." The grace/works mutual-exclusivity engine applied directly to election. Suggested filename: `apologetic-the-remnant-chosen-by-grace.html`.
- **Definite Atonement via Isaiah 53:11-12** — "by his knowledge my righteous servant will justify *many*... he bore the sin of *many*." The Servant's definite "many" (Hebrew *rabbim*) named in the great substitution chapter — an Old-Testament companion to save-his-people and for-the-sheep. Suggested filename: `apologetic-he-bore-the-sin-of-many.html`.
- **Irresistible Grace via Titus 3:5 / regeneration before faith** — "he saved us through the washing of rebirth (*palingenesia*) and renewal by the Holy Spirit" — the new birth as God's act preceding and producing faith. Suggested filename: `apologetic-the-washing-of-rebirth.html`.
- **Total Depravity via Genesis 6:5 / 8:21** — "every inclination of the thoughts of the human heart was only evil all the time" — the flood diagnosis (Hebrew *yetzer*, *raq ra‘ kol-hayom*), depravity stated at maximum scope before any law was given. Suggested filename: `apologetic-only-evil-all-the-time.html`.

Generate ONE net-new article (~2500-3500 words); ship more if context allows (Sessions 46-47 each shipped 5). Pre-Glob-verify every internal href before writing. Cross-link the existing THIRTY-ONE Five-Point Proliferation articles so the diamond stays visible from inside any new article.
...
- Re-run `node audit-prose.js` after the batch; document score-change per new article + net-new count + sample-read-fail-fix count in the Session 48 Log entry.
- 8-step pipeline run ONCE at end (all 8 GREEN). Subagent recommended.
- Session 47 Log entry exists; Session 48 entry is added; Session 49 starts-here block at bottom.
- Status line and Phase table updated.
- Lock count documented (authoritative at S47 close: 298 per-type / 289 distinct); may rise (new lock) or fall (revoked marker) — both valid, both must be logged. Spot-check the 182→200 POLISH drift noted above.
