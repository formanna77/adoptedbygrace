...
**Next action:** Session 46 — **Phase F + Phase G + Landing-Force Audit continue in parallel. Phase D remains paused.** Session 45 shipped ~5 article-equivalents (mid-stretch): four net-new apex Five-Point builds (`apologetic-no-one-can-come-unless` IG #4 via John 6 *helkyō*; `apologetic-nothing-can-separate` PoS #4 via Romans 8:31-39; `apologetic-the-conscience-that-only-accuses` TD §XIII.2 via Romans 2:14-15; `apologetic-for-the-sheep` DA #5 via John 10:11-15) + a Phase F banned-move surgical fix stripping the stale "3:14 in the morning" card snippet from 7 files + a clean 6-page Landing-Force sample-read (all PASS). All 8 validators GREEN; all four discipline axes clean site-wide. **Five-Point coverage post-Session-45: TD 4, UE 4, DA 5, IG 4, PoS 4 — 21 Five-Point apologetics on disk, fully balanced.** The diamond is visible from twenty-one adjacent facets. Session 46's recommended Priority 1 candidates: **(A) Total Depravity via Romans 8:7-8** — the carnal mind's hostility + double "cannot" (*oude gar dynatai*); **(B) Unconditional Election #5 via John 17** — "those you gave me" (*hous dedōkas moi*); **(C) Irresistible Grace #5 via Acts 9 / the Damascus road** — the wholly involuntary conversion as paradigm of the effectual call. Phase F audit-prose §II queue continues as backup. Floor 4 article-equivalents (any axis counts). Stretch 6-10. See "AUDIT 2 / Session 46 starts here" at bottom.
...
## AUDIT 2 / Session 46 starts here

**🟢 PHASE F + PHASE G CONTINUE IN PARALLEL. PHASE D EXPANSION REMAINS PAUSED.** Session 45 closed at ~5 article-equivalents (above floor 4, into stretch): FOUR net-new apex Five-Point builds + ONE Phase F banned-move surgical-fix cluster across 7 files + a clean 6-page Landing-Force sample-read. Net-new: `apologetic-no-one-can-come-unless.html` (IG #4 via John 6:44-65 *helkyō* — the hauling verb + Bread-of-Life discourse architecture + the John 6:66 schism), `apologetic-nothing-can-separate.html` (PoS #4 via Romans 8:31-39 — the seven *tis* questions + the *ktisis*-includes-you catalogue), `apologetic-the-conscience-that-only-accuses.html` (TD §XIII.2 via Romans 2:14-15 *symmartyrousēs* — the conscience that convicts but cannot empower), `apologetic-for-the-sheep.html` (DA #5 via John 10:11-15 *hyper tōn probatōn* + the v26 bracket). Surgical fix: stripped the stale banned-move `"It is 3:14 in the morning. "` card-snippet prefix from 7 files (objection-manipulation, pastoral-prayer, pastoral-when-truth-lands, philosophy-gravity-of-grace, philosophy-sleep-surrender, philosophy-what-if-wrong, story-the-letter-before-the-world). All eight pipeline validators GREEN; all FOUR discipline axes clean site-wide (slogan-trap 0; clock-time 0; structural 0; stale 3:14-cards 0). Lock count: 286.

**Read these files FIRST, in order:**
1. **`MISSION-CONTROL.md`** Session 45 entry (above this block). It documents the four net-new builds, the 7-file banned-move surgical fix (with the SURPRISE finding + standing note on stale `related-article-desc` cards), and the all-PASS sample-read. **Inheritance reminder: before any net-new build, Glob `apologetic-*.html` against the inventory below and verify any unexplained-recent files apex-grade before writing on the same topic (the Session-44 near-redo lesson).**
2. **`VOICE.md`** §XIII (Depravity Concrete-Unpacking Mandate) and §XIV (Five-Point Proliferation Mandate). Session 46 continues both. Five-Point coverage post-Session-45: TD 4 (Lazarus, Cardiology, Prayer-diagnostic, Conscience), UE 4 (eklogē, Ephesians 1 eulogy, Acts 13:48, Deuteronomy 7), DA 5 (priest-shoulders, Owen, mercy seat, ephapax, for-the-sheep), IG 4 (Lydia, revival, cardiac transplant, helkyō/John 6), PoS 4 (arrabōn, unbroken chain, double-grip, nothing-can-separate). **21 Five-Point apologetics on disk — fully balanced, every point at 4+, DA strongest at 5.** The diamond is visible from twenty-one adjacent facets.
3. **`PROSE-INTEGRITY.md`** — the Phase F gating artifact. The audit-prose-driven axis remains active; all four banned-move/discipline axes confirmed clean Session 45 close.
...
**Session 46 task — Phase F + Phase G + Landing-Force Audit, in parallel.** Floor: 4 article-equivalents (any axis counts; all four axes equal-yield). Stretch: 6-10. Ceiling-stretch: 12+. **Open the session by Glob-checking `apologetic-*.html` against the Session-45 inventory (21 Five-Point + 5 older = 26 total expected); if Session 46 finds any net-new files since this entry, verify their quality and add them to the closing-log inventory before counting toward floor.**
...
Suggested sample-pool seed for Session 46 (ROTATE — Session 45 already read devotional-never-gives-up, question-faithgift, question-depravity + analogy-doctor-corpse, psychology-system-justification, history-golden-thread, all PASS): draw fresh — `systematic-election` (309 inbound); `question-chosen` (307); `devotional-in-christ` (254); `question-irresistible`; `question-perseverance`; `systematic-tulip`; plus 3–4 random unmarked pages drawn by walking `ls *.html | grep -v -e 'POLISH-LOCKED' …` (or spot-picking from the ~216 unmarked content pages). **The three unmarked pages Session 45 read (analogy-doctor-corpse, psychology-system-justification, history-golden-thread) were PROVEN apex and are marker-eligible if a future session wants to lock them.**
...
This catches the Session 41 v3 detector candidate (e) — the structural pattern even when the strong noun is not "Dead." Session 43 surfaced one such violation in Session 42's Lazarus article (applied to *nekrous*). Any new occurrence is a Session-46 highest-priority fix.

**Priority 1 — Phase G Five-Point Proliferation continuation.** Post-Session-45 coverage:

| Point | Apologetics shipped | Status |
|---|---:|---|
| Total Depravity | 4 (Lazarus + Cardiology + Prayer-diagnostic + Conscience) | strong |
| Unconditional Election | 4 (*eklogē* + Ephesians 1 eulogy + Acts 13:48 *tetagmenoi* + Deuteronomy 7) | strong |
| Definite Atonement | 5 (priest-shoulders + Owen Trilemma + mercy seat + *ephapax* + for-the-sheep) | strongest |
| Irresistible Grace | 4 (Lydia + revivals + cardiac transplant + *helkyō*/John 6) | strong |
| Perseverance | 4 (*arrabōn* + unbroken chain + double-grip + nothing-can-separate) | strong |

**Five-Point Proliferation is now fully balanced — every point at 4+, DA strongest at 5.** All four Session-45 recommended candidates are shipped. Session 46's recommended Priority 1 candidates (each from an angle the site does not yet cover — pre-Glob-verify each filename does not already exist):
- **Total Depravity via Romans 8:7-8 (the carnal mind's double "cannot")** — *to phronēma tēs sarkos echthra eis theon* ("the mind governed by the flesh is hostile to God") + *oude gar dynatai* ("nor can it [submit]") + "those in the flesh cannot please God." The hostility-PLUS-inability angle that pairs with the conscience article (knowing-vs-doing) by adding the will's enmity. Suggested filename: `apologetic-the-mind-that-cannot-submit.html`.
- **Unconditional Election #5 via John 17 (the given ones)** — the High-Priestly prayer's relentless "those you gave me" / *hous dedōkas moi* refrain (vv2, 6, 9, 24), the people given by the Father to the Son before the foundation of the world. Pairs with for-the-sheep (the John 17:9 thread) and tetagmenoi. Suggested filename: `apologetic-those-you-gave-me.html`.
- **Irresistible Grace #5 via Acts 9 / the Damascus road** — Paul's wholly involuntary conversion as the paradigm case of the effectual call: the persecutor knocked down, blinded, and remade with no prevenient cooperation. A history/testimony-register companion to the *helkyō* Greek-register build. Suggested filename: `apologetic-the-road-to-damascus.html`.
- **Alternative — Perseverance #5 via Philippians 1:6 / Jude 24** — "He who began a good work in you will carry it on to completion" + "to him who is able to keep you from stumbling," the divine-completion bookends.

Generate ONE net-new article (~2500-3500 words). Pre-Glob-verify every internal href before writing. Cross-link to the existing TWENTY-ONE Five-Point Proliferation articles (priest-shoulders, *arrabōn*, Lydia, *eklogē*, Owen Trilemma, Lazarus, unbroken chain, revival, eulogy of Ephesians 1, mercy seat, *ephapax*, cardiac transplant, double-grip, cardiology of the fall, *tetagmenoi*, prayer-impulse, Deuteronomy 7, *helkyō*/no-one-can-come-unless, nothing-can-separate, conscience-that-only-accuses, for-the-sheep) so the diamond stays visible from inside any new article.
...
- Re-run `node audit-prose.js` after the batch and document score-change per article (and net-new-article count, and sample-read-fail-fix count) in the Session 46 Log entry.
- 8-step pipeline run ONCE at end (all 8 GREEN). Subagent recommended to keep pipeline output out of main context.
- Session 45 Log entry exists; Session 46 entry is added; Session 47 starts-here block at bottom.
- Status line and Phase table updated.
- Lock count documented (current 286 at Session 45 close); may rise if a Phase C consecration ships in parallel; may FALL if sample-read revokes a marker — both directions are valid and must be logged.
