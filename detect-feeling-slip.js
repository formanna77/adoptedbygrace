...
// ---------------------------------------------------------------------------
// S209 — THE LAUNDERED CERTIFICATE.
//
// For eight sessions the REPUDIATE/DENIED branch has run first and cleared any
// sentence containing a denial. S207 hunted this class by hand and found six
// live certificates the detector had cleared — one of them on the doorway built
// for skeptics. The reason is exact:
//
//     "The ache you feel for a God you think you have lost is NOT EVIDENCE that
//      you have been abandoned — it is the trace of having been reached."
//
// The denial is real. It denies ABANDONMENT. The clause after the pivot then
// issues a certificate on the same feeling, and the `not evidence` token
// launders it through the first branch. A denial does not launder a
// certificate: what matters is WHAT THE DENIAL DENIES.
//
// The test below takes the residue AFTER the last denial, requires a
// contrastive pivot (— ; but / rather / instead / it is), and asks whether that
// residue independently re-issues a certificate.
//
// IT DOES NOT RECLASSIFY TO SLIP, AND THAT IS DELIBERATE. §XXIII.2 is explicit
// that no phrase-grep separates a slip from a sanctioned interior paragraph,
// because the words are identical and only the object differs — and this site
// has already ruled one member of this exact shape a sanctioned repair
// ("The wanting is not YOUR proof; it is His fingerprint", S183). Folding the
// class into SLIP would trade eight sessions of false negatives for a season of
// false positives and corrupt the ratchet besides. So LAUNDERED is its own
// column: the detector stops SILENTLY CLEARING the class — which was the whole
// defect — and hands each one to a human eye instead of a verdict.
const RESIDUE_CERT =
  /\b(?:it|that|this)(?:'s| is| was)\b[^.!?<]{0,90}\b(?:the )?(?:trace|mark|print|fingerprint|imprint|echo|residue|footprint|signature|first sign|beginning|stirring|pull|drawing|call|summons|proof|evidence|sign)\b/i
  .source + '|' + /\bhaving been (?:reached|drawn|found|chosen|called|held|kept|sought|touched|awakened)\b/i.source
  + '|' + /\b(?:it|that|this)(?:'s| is| was) (?:already )?(?:Him|God|Christ|the Spirit|the Father|grace|mercy|the Shepherd)\b/i.source
  + '|' + /\bmeans (?:that )?you (?:are|have been|were)\b/i.source;
const LAUNDER_RESIDUE = new RegExp(RESIDUE_CERT, 'i');
const PIVOT = /[—–]|;|\b(?:but|rather|instead|yet|it is|it's|that is|that's)\b/i;

function launderedCertificate(s) {
  let idx = -1;
  for (const re of [REPUDIATE, DENIED, DENIED_APPOS, NAMED_TRICK, DIAGNOSED]) {
    if (!re) continue;
    re.lastIndex = 0;
    const m = re.exec(s);
    if (m) idx = Math.max(idx, m.index + m[0].length);
  }
  if (idx < 0) return false;
  const tail = s.slice(idx);
  if (!PIVOT.test(tail)) return false;
...
function scan(file) {
  ...
  for (const s of sentencesOf(raw)) {
    ...
    // S209: the laundering test runs in FRONT of the denial branch — it is the
    // one case where a denial is not a repudiation. Everything below is untouched
