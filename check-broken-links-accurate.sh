...
# Accounts for URL rewriting: /about -> about.html
# Output: TSV format: source_file \t broken_href \t resolved_target \t exists
...
echo -e "source_file\tbroken_href\tresolved_target\texists"
while IFS='|' read -r source_file href; do
    ...
    if ! check_link_exists "$resolved"; then
        ...
        echo -e "$source_file\t$href\t$resolved\tNO"
    fi
